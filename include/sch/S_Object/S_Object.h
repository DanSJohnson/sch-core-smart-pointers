 ...
 * the time stamp is used to check if objects moved or not between two proximity queries, if they didn't, computing the
 * distance between them becomes useless.
 */
...
public:
  /*!
   *  \brief gives the support point for a given vector in local cordinates and a given last feature. MUST BE
   * OVERLOADED. \param v direction vector (normalized) \param lastFeature tells in which feature we were in last query,
   * amd returns the new one. very important in spatio-temporal coherence \return returns the support point.
   */
...
public:
  ...
  /*!
   *  \brief put it in objects coordinates, then calls l_support. DO NOT OVERLOAD IT UNLESS YOU KNOW WHAT YOU ARE DOING
   * (if you want to define the support function, overload l_support) \param v direction vector \return returns the
   * support point.
   */
   ...
   *  \param lastFeature tells in which feature we were in last query, and returns the new one. very important in
   * spatio-temporal coherence \return returns the support point.
   */
